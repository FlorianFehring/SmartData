...
/**
 * Filter class for within geofilter
 *
 * @author Alexej Rogalski
 */
public class WithinGeoFilter extends Filter {
     
    private String geometry = "";
    

    public WithinGeoFilter(DynCollection table) {
        super(table);
    }
            ...
            switch (col.getType()) {
                ...
                default:
                    Message msg = new Message(
                            "WithinGeoFilter", MessageLevel.ERROR,
                            "Column type >" + col.getType() + "< is not supported. Please choose a Column with type geometry.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse WithinGeoFilter: " + ex.getLocalizedMessage());
