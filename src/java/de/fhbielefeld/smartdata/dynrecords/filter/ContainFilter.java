...
/**
 * Filter class for contain filters
 *
...
public class ContainFilter extends Filter {
    ...
    public ContainFilter(DynCollection table) {
        super(table);
    }
    ...
    public void parse(String filtercode) throws FilterException {
        ...
        try {
            ...
            // Thrid element is the value that should be contain
            switch (col.getType()) {
                case "text":
                case "varchar":
                    this.csvalue = parts[2];
                    break;
                default:
                    Message msg = new Message(
                            "ContainFilter", MessageLevel.WARNING,
                            "Column type >" + col.getType() + "< is currently not supported.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse ContainFilter: " + ex.getLocalizedMessage());
    ...
    public PreparedStatement setFilterValue(PreparedStatement pstmt) throws FilterException {
        ...
        try {
            pstmt.setString(pos, (String) this.csvalue);
