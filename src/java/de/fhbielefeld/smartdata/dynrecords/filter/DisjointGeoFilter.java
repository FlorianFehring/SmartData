...
/**
 * Filter class for disjoint geofilter
 *
 * @author Alexej Rogalski
 */
public class DisjointGeoFilter extends Filter {
     
    private String geometry = "";
    

    public DisjointGeoFilter(DynCollection table) {
        super(table);
    }
            ...
            switch (col.getType()) {
                ...
                default:
                    Message msg = new Message(
                            "DisjointGeoFilter", MessageLevel.ERROR,
                            "Column type >" + col.getType() + "< is not supported. Please choose a Column with type geometry.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse DisjointGeoFilter: " + ex.getLocalizedMessage());
    ...
    public String getPrepareCode() {
        return "ST_Disjoint(" + "\"" + this.collection.getName() + "\".\"" + this.attribute + "\"" + ", ST_GeomFromText(?))";
    }
