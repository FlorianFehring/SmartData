...
/**
 * Filter class for touches geofilter
 *
 * @author Alexej Rogalski
 */
public class TouchesGeoFilter extends Filter {
     
    private String geometry = "";
    

    public TouchesGeoFilter(DynCollection table) {
        super(table);
    }
            ...
            switch (col.getType()) {
                ...
                default:
                    Message msg = new Message(
                            "TouchesGeoFilter", MessageLevel.ERROR,
                            "Column type >" + col.getType() + "< is not supported. Please choose a Column with type geometry.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse TouchesGeoFilter: " + ex.getLocalizedMessage());
