...
/**
 * Filter class for overlaps geofilter
 *
 * @author Alexej Rogalski
 */
public class OverlapsGeoFilter extends Filter {
     
    private String geometry = "";
    

    public OverlapsGeoFilter(DynCollection table) {
        super(table);
    }
            ...
            switch (col.getType()) {
                ...
                default:
                    Message msg = new Message(
                            "OverlapsGeoFilter", MessageLevel.ERROR,
                            "Column type >" + col.getType() + "< is not supported. Please choose a Column with type geometry.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse OverlapsGeoFilter: " + ex.getLocalizedMessage());
