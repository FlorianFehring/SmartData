...
/**
 * Filter class for contains geofilter
 *
 * @author Alexej Rogalski
 */
public class ContainsGeoFilter extends Filter {
     
    private String geometry = "";
    

    public ContainsGeoFilter(DynCollection table) {
        super(table);
    }
            ...
            switch (col.getType()) {
                ...
                default:
                    Message msg = new Message(
                            "ContainsGeoFilter", MessageLevel.ERROR,
                            "Column type >" + col.getType() + "< is not supported. Please choose a Column with type geometry.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse ContainsGeoFilter: " + ex.getLocalizedMessage());
