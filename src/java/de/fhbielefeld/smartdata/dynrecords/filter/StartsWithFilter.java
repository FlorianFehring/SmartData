...
/**
 * Filter class for start with filters
 *
 * @author Alexej Rogalski
 */
public class StartsWithFilter extends Filter {

    private Object swvalue;

    public StartsWithFilter(DynCollection table) {
        super(table);
    }
    ...
    public void parse(String filtercode) throws FilterException {
        ...
        try {
            ...
            // Thrid element is the value that should be at the begin
            switch (col.getType()) {
                case "text":
                case "varchar":
                    this.swvalue = parts[2];
                    break;
                default:
                    Message msg = new Message(
                            "StartsWithFilter", MessageLevel.WARNING,
                            "Column type >" + col.getType() + "< is currently not supported.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse StartsWithFilter: " + ex.getLocalizedMessage());
    ...
    public PreparedStatement setFilterValue(PreparedStatement pstmt) throws FilterException {
        ...
        try {
            pstmt.setString(pos, (String) this.swvalue);
