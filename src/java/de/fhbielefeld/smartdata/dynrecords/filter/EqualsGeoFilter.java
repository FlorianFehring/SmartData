...
/**
 * Filter class for equals geofilter
 *
 * @author Alexej Rogalski
 */
public class EqualsGeoFilter extends Filter {
     
    private String geometry = "";
    

    public EqualsGeoFilter(DynCollection table) {
        super(table);
    }
            ...
            switch (col.getType()) {
                ...
                default:
                    Message msg = new Message(
                            "EqualsGeoFilter", MessageLevel.ERROR,
                            "Column type >" + col.getType() + "< is not supported. Please choose a Column with type geometry.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse EqualsGeoFilter: " + ex.getLocalizedMessage());
    ...
    public String getPrepareCode() {
        return "ST_Equals(" + "\"" + this.collection.getName() + "\".\"" + this.attribute + "\"" + ", ST_GeomFromText(?))";
    }
