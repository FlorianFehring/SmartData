...
/**
 * Filter class for crosses geofilter
 *
 * @author Alexej Rogalski
 */
public class CrossesGeoFilter extends Filter {
     
    private String geometry = "";
    

    public CrossesGeoFilter(DynCollection table) {
        super(table);
    }
            ...
            switch (col.getType()) {
                ...
                default:
                    Message msg = new Message(
                            "CrossesGeoFilter", MessageLevel.ERROR,
                            "Column type >" + col.getType() + "< is not supported. Please choose a Column with type geometry.");
                    Logger.addDebugMessage(msg);
            }

        } catch (DynException ex) {
            FilterException fex = new FilterException("Could not parse CrossesGeoFilter: " + ex.getLocalizedMessage());
